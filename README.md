...
```
 ...
 8. Select **Save**. You should see the interaction model being built (this might a take a minute or 2). If you select next, your changes will be saved and you will go directly to the test screen. After selecting Save, it should now look like this:
...
**Note: If you are new to Lambda and would like more information, visit the [Lambda Getting Started Guide](http://docs.aws.amazon.com/lambda/latest/dg/getting-started.html).**
 ...
 1. **IMPORTANT**: For Regions (upper right) , Select **US East (N. Virginia)** for US skills and **EU (Ireland)** for UK/DE skills. These are the only two regions that currently supports Alexa Skill development, and choose the right region to guarantee better laterncy.
 ...
 ![](https://s3.amazonaws.com/lantern-code-samples-images/fact/aws_blueprint.png)
...
4. For better latency, deploying your code to different endpoints is recommended. Follow the Create Lambda Function instructions in Step 2 and be sure to select a right lambda region. Select **US East (N. Virginia)** for US skills and **EU (Ireland)** for UK/DE skills. Copy the ARN for use in the Configuration section of the Amazon Developer Portal.

5. Go back to Skill Configuration section. which is marked as global fields for all language. Add extra endpoint and paste your Lambda ARN. Save your skill configuration infromation.
...
For Global fields, choose an proper category. IMPORTANT: Add the text “This is based on the Fact Skill Template” to the Testing Instructions section. This alerts the Certification team of your submission using this standardized template, smoothing the road to a faster publish. And also select the countries that you want your skill to be available.
...
4. (Optional) For multilple language skill, once finish and save publishing infomation for your initial language, you will need to do it again for your second language. Under your second language tab, select Publishing Infomation, add additional publishing region(s) at global field and all other customer facing information in non-global field.
...
6. Select **“Save”**. If your skill supports multiple languages, then you will need to compelete Privacy and Compliance for each language before submission.
